...
void calculateTime(float dm1, float dm2){
	...
	// if(abs(dm1) > abs(dm2)){
	//  	time = abs(dm1) / feedrate; // time = abs(dm2) / frMMs;
	// }else{
	//  	time = abs(dm2) / feedrate; // time = abs(dm2) / frMMs; 
	//  }

    if(abs(dm1) > abs(dm2)){
        time = abs((dm1 / feedrate) * 1000000) ; // Multiply with 1000 for microseconds
        delayM1 = time / stepsM1;
        delayM2 = time / stepsM2;
        //Serial.print("TotalTIme = ");Serial.print(time / 1000);Serial.println(" MS");	 

	}else{
        time = abs((dm2 / feedrate) * 1000000) ; // Multiply with 1000 for miliseconds
        delayM1 = time / stepsM1;
        delayM2 = time / stepsM2;
        //Serial.print("TotalTIme = ");Serial.print(time / 1000);Serial.println(" MS");	 
    }
	...
	// delayM1 = abs((1000 / 1000) / stepsM1);  // delayM1 = abs((time * feedrate) / stepsM1);
	// delayM2 = abs((time * 1000) / stepsM2); // delayM1 = abs((time * feedrate) / stepsM1);
...
void drawLine(float x, float y){
	...
	unsigned long timeOfArrival = currentTime + time;
    //Serial.print("Estimated Time to arrival = "); Serial.println(timeOfArrival-currentTime / 1000);
	unsigned long timerM1 =  currentTime + delayM1 ;
	unsigned long timerM2 =  currentTime + delayM2 ;
    //Serial.print("timerM1 = ");Serial.print(timerM1);Serial.print("    timerM2 = ");Serial.println(timerM2);

    float accel = 1;
    float accelStep = 0.002;
    float decStep = 0.001;
    int stepsAccelM1 = stepsM1 * 0.2;
    int stepsAccelM2 = stepsM2 * 0.2;
    int stepsDecM1 = stepsM1 * 0.2;
    int stepsDecM2 = stepsM2 * 0.2;
    int acelM1Steps = stepsM1;
    int acelM2Steps = stepsM2;

    while(stepsM1 > 0 || stepsM2 > 0){
		float now = micros();
		if(now >= timerM1 && stepsM1 > 0){
			if(m1Dir == 0){
				L1 -= stepToMM;
                stepM1(0);
			}else{
			...
			timerM1 = now + (delayM1 + (delayM1 * accel));// + acceleration;
            if(accel > 0 && stepsM1 > acelM1Steps - stepsAccelM1){
                accel -= accelStep;
            }
            if(stepsM1 < stepsDecM1){
                accel += decStep;
            } 
            stepsM1--;
			FK(L1,L2,posX,posY);
		}
		if(now >= timerM2 && stepsM2 > 0){
			if(m2Dir == 0){
				L2 -= stepToMM;
                stepM2(0);
			}else{
			...
			timerM2 = now + (delayM2 + (delayM2 * accel));// + acceleration;
            if(accel > 0 && stepsM2 > acelM2Steps - stepsAccelM2){
                accel -= accelStep;
            } 
            if(stepsM2 < stepsDecM2){
                accel += decStep;
            } 
            stepsM2--;
			FK(L1,L2,posX,posY);
...
void initMotors(){
    ...
    stepsInOneSec = feedrate / stepToMM;
     ////// PolarBot ///////////
    Serial.print("spoolCir =  ");Serial.println(spoolCir);
    Serial.print("stepToMM =  ");Serial.println(stepToMM);
    Serial.print("stepsInOneSec =  ");Serial.println(stepsInOneSec);
    //Serial.print("frMMs =  ");Serial.println(frMMs);
